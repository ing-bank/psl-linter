...
describe('Parameter tests', () => {
	...
	test('One diagnostic second start', () => {
		...
		expect(reports[0].message).toBe(
			'Declaration "flagged" referenced inside Runtime.start but not ' +
			'in variable list.'
		);
	});
	...
	test('Method in middle with new variable', () => {
		...
		expect(reports.length).toBe(1);
		expect(reports[0].message).toBe(
			'Parameter "flaggedParam" referenced inside Runtime.start but not ' +
			'in variable list.'
		);
	});
	...
	test('Method in middle with new variable', () => {
		...
		expect(diagnostic.message).toBe(
			'Declaration "notFlaggedTwice" referenced inside Runtime.start but not in' +
			' variable list.',
		);
