...
describe('Parameter tests', () => {
	...
	test('allProblemsNoLineAbove', () => {
		expect(messageOnLine(0, docDiagnostics))
			.toBe('Documentation missing for label "allProblemsNoLineAbove".');
		expect(messageOnLine(0, separatorDiagnostics))
			.toBe('Separator missing for label "allProblemsNoLineAbove".');
		expect(messageOnLine(0, emptyLineDiagnostics)).toBe(
			'There should be two empty lines above label "allProblemsNoLineAbove".'
			);
	});
	test('allProblems', () => {
		expect(messageOnLine(2, docDiagnostics))
			.toBe('Documentation missing for label "allProblems".');
		expect(messageOnLine(2, separatorDiagnostics))
			.toBe('Separator missing for label "allProblems".');
		expect(messageOnLine(2, emptyLineDiagnostics))
			.toBe('There should be two empty lines above label "allProblems".');
	});
	test('onlySeparator', () => {
		expect(messageOnLine(5, docDiagnostics))
			.toBe('Documentation missing for label "onlySeparator".');
		expect(messageOnLine(5, separatorDiagnostics)).toBe('');
		expect(messageOnLine(5, emptyLineDiagnostics))
			.toBe('There should be two empty lines above label "onlySeparator".');
	});
	test('twoLineSeparator', () => {
		expect(messageOnLine(14, docDiagnostics))
			.toBe('Documentation missing for label "twoLineSeparator".');
	...
	test('onlyDoc', () => {
		...
		expect(messageOnLine(16, separatorDiagnostics))
			.toBe('Separator missing for label "onlyDoc".');
		expect(messageOnLine(16, emptyLineDiagnostics))
			.toBe('There should be two empty lines above label "onlyDoc".');
	});
	test('oneLineDoc', () => {
		expect(messageOnLine(20, docDiagnostics)).toBe('');
		expect(messageOnLine(20, separatorDiagnostics))
			.toBe('Separator missing for label "oneLineDoc".');
		expect(messageOnLine(20, emptyLineDiagnostics))
			.toBe('There should be two empty lines above label "oneLineDoc".');
	});
	test('twoLineDoc', () => {
		expect(messageOnLine(25, docDiagnostics)).toBe('');
		expect(messageOnLine(25, separatorDiagnostics))
			.toBe('Separator missing for label "twoLineDoc".');
